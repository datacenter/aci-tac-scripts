...
function getTacRecord() {
    	...
    	count=`icurl 'http://localhost:7777/api/class/'"$record"'.xml?query-target-filter=and(gt('"$record"'.'"$sortAtt"',"'"$SDATE"'"),lt('"$record"'.'"$sortAtt"',"'"$EDATE"'"))&rsp-subtree-include=count' 2>/dev/null | sed 's/^.*count="//g' | sed 's/".*$//g' 2> /dev/null`
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	if ! egrep -ql "^$line\." /tmp/objects.txt; then
			...
			record=`egrep "^$line\." /tmp/objects.txt | awk -F " " '{print $2}'`
				if [ "$record" == 'faultInfo' ] || [ "$record" == 'healthInst' ] || [ "$record" == 'aaaModLR' ]; then
					...
					if [[ $record == "faultInfo" ]] || [[ $record == "aaaModLR" ]]; then
							sortAtt=created
							SDATE="2010-12-15T00:00:00"
							EDATE="2050-12-15T00:00:00"
							getTacRecord
...
###Collect additional implicit MO's that can't be sorted by created attribute
if [[ $disable_implicit != "yes" ]]; then
